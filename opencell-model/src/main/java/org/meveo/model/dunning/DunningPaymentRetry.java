...
@GenericGenerator(name = "ID_GENERATOR", type = org.hibernate.id.enhanced.SequenceStyleGenerator.class, parameters = { @Parameter(name = "sequence_name", value = "dunning_payment_retries_seq"), @Parameter(name = "increment_size", value = "1") })
@NamedQueries({ @NamedQuery(name = "DunningPaymentRetry.findByPaymentMethodAndPsp", query = "select d FROM DunningPaymentRetry d where d.paymentMethod = :paymentMethod and d.psp = :psp") })
...
public class DunningPaymentRetry extends AuditableEntity {
    ...
    public DunningPaymentRetry(@Size(max = 100) PaymentMethodEnum paymentMethod, @Size(max = 255) String psp, Integer numPayRetries, @Size(max = 255) PayRetryFrequencyUnitEnum payRetryFrequencyUnit,
            Integer payRetryFrequency, @NotNull DunningSettings dunningSettings) {
        super();
