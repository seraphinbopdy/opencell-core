...
@GenericGenerator(name = "ID_GENERATOR", type = org.hibernate.id.enhanced.SequenceStyleGenerator.class, parameters = { @Parameter(name = "sequence_name", value = "order_article_line_seq"), @Parameter(name = "increment_size", value = "1") })
@NamedQueries({ @NamedQuery(name = "OrderArticleLine.findByOrderId", query = "select op FROM OrderArticleLine op WHERE op.order.id = :commercialOrderId") })
