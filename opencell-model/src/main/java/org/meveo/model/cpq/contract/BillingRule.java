...
@GenericGenerator(name = "ID_GENERATOR", type = org.hibernate.id.enhanced.SequenceStyleGenerator.class, parameters = { @Parameter(name = "sequence_name", value = "cpq_billing_rule_seq"), @Parameter(name = "increment_size", value = "1") })
@NamedQueries({
        @NamedQuery(name = "BillingRule.findByAccounts", query = "select br from BillingRule br where "
                ...
                + " and (br.contract.billingAccount.id is null or br.contract.billingAccount.id=:billingAccountId) " + " and (br.contract.customerAccount.id is null or br.contract.customerAccount.id=:customerAccountId) "
                + " and (br.contract.customer.id is null or br.contract.customer.id=:customerId) " + " and (br.contract.seller.id is null or br.contract.seller.id=:sellerId) "
        ...
        @NamedQuery(name = "BillingRule.findByAccountsWithSellerNull", query = "select br from BillingRule br where "
                + " br.contract.status = 'ACTIVE' and (br.contract.beginDate <= current_date and br.contract.endDate > current_date) "
                + " and (br.contract.billingAccount.id is null or br.contract.billingAccount.id=:billingAccountId) " + " and (br.contract.customerAccount.id is null or br.contract.customerAccount.id=:customerAccountId) "
                + " and (br.contract.customer.id is null or br.contract.customer.id=:customerId) " + " and (br.contract.seller.id is null) "
                + " order by br.contract.billingAccount.id, br.contract.customerAccount.id, br.contract.customer.id, br.contract.seller.id, br.priority NULLS LAST"),
        @NamedQuery(name = "BillingRule.findByAccountsAndContract", query = "select br from BillingRule br where " + " br.contract.id=:contractId "
                + " and (br.contract.billingAccount.id is null or br.contract.billingAccount.id=:billingAccountId) " + " and (br.contract.customerAccount.id is null or br.contract.customerAccount.id=:customerAccountId) "
                + " and (br.contract.customer.id is null or br.contract.customer.id=:customerId) " + " and (br.contract.seller.id is null or br.contract.seller.id=:sellerId) "
                + " order by br.contract.billingAccount.id, br.contract.customerAccount.id, br.contract.customer.id, br.contract.seller.id, br.priority NULLS LAST"),
        @NamedQuery(name = "BillingRule.findByAccountsAndContractWithSellerNull", query = "select br from BillingRule br where " + " br.contract.id=:contractId "
                + " and (br.contract.billingAccount.id is null or br.contract.billingAccount.id=:billingAccountId) " + " and (br.contract.customerAccount.id is null or br.contract.customerAccount.id=:customerAccountId) "
                + " and (br.contract.customer.id is null or br.contract.customer.id=:customerId) " + " and (br.contract.seller.id is null) "
                + " order by br.contract.billingAccount.id, br.contract.customerAccount.id, br.contract.customer.id, br.contract.seller.id, br.priority NULLS LAST"),
