...
@NamedQueries({ @NamedQuery(name = "Product.getProductLine", query = "select p from Product p where p.productLine.id=:id"),
...
public class Product extends ServiceCharge {
    ...
    @JoinTable(name = "cpq_product_discount_plan", joinColumns = @JoinColumn(name = "product_id", referencedColumnName = "id"), inverseJoinColumns = @JoinColumn(name = "discount_id", referencedColumnName = "id"))
    private Set<DiscountPlan> discountList = new HashSet<>();

    /**
     * flag that indicate if true discount list will have a specific list otherwise all available discount attached to this product will be displayed
     */
    ...
    public List<ServiceChargeTemplateTermination> getServiceTerminationCharges() {
        ...
        for (ProductChargeTemplateMapping pc : getProductCharges()) {
            if (pc.getChargeTemplate() != null) {
                ChargeTemplate ch = initializeAndUnproxy(pc.getChargeTemplate());
                if (ch instanceof OneShotChargeTemplate && ((OneShotChargeTemplate) ch).getOneShotChargeTemplateType() == OneShotChargeTemplateTypeEnum.TERMINATION) {
    ...
    public int hashCode() {
        ...
        result = prime * result + Objects.hash(brand, discountFlag, discountList, model, modelChildren, productLine, reference, status, statusDate);
        return result;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(brand, other.brand) && discountFlag == other.discountFlag && Objects.equals(discountList, other.discountList) && Objects.equals(model, other.model)
                && Objects.equals(modelChildren, other.modelChildren) && Objects.equals(productLine, other.productLine) && Objects.equals(reference, other.reference) && status == other.status
