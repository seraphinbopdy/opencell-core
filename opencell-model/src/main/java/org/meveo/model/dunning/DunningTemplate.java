...
@NamedQueries(@NamedQuery(name = "DunningTemplate.isDunningTemplatedRelatedToAnActiveDunningLevel", query = "SELECT dt FROM DunningTemplate dt "
        + "JOIN DunningAction da ON da.actionNotificationTemplate.id = :templateId " + "JOIN DunningLevel dl ON dl MEMBER OF da.relatedLevels " + "WHERE dt.id=:templateId AND dl.isActive=true "))
