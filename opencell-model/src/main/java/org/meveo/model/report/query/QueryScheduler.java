...
@GenericGenerator(name = "ID_GENERATOR", type = org.hibernate.id.enhanced.SequenceStyleGenerator.class, parameters = { @Parameter(name = "sequence_name", value = "query_scheduler_seq"), @Parameter(name = "increment_size", value = "1") })
@NamedQueries({ @NamedQuery(name = "QueryScheduler.findByReportQuery", query = "select qs from QueryScheduler qs where qs.reportQuery = :reportQuery") })
