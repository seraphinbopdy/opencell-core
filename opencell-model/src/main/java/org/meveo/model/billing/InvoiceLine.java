...
@NamedQueries({ @NamedQuery(name = "InvoiceLine.listToInvoiceByBillingAccountAndIDs", query = "select il FROM InvoiceLine il where il.billingAccount.id=:billingAccountId AND il.status='OPEN' AND id in (:listOfIds) "),
        ...
        @NamedQuery(name = "InvoiceLine.getInvoicingItems", query =
				"select il.billingAccount.id, il.accountingArticle.invoiceSubCategory.id, il.userAccount.id, il.tax.id, il.seller.id, il.invoiceType.id, sum(il.amountWithoutTax), sum(il.amountWithTax), sum(il.amountTax), count(il.id), (string_agg(cast(il.id as text),',')),"
						...
						+ " group by il.billingAccount.id, il.accountingArticle.invoiceSubCategory.id, il.userAccount.id, il.tax.id, il.seller.id, il.invoiceType.id, il.invoiceKey "
		...
		@NamedQuery(name = "InvoiceLine.updateByIncrementalModeWoutDatesWithAverageUnitAmountsAndQuantityZERO", query = "UPDATE InvoiceLine il SET il.amountWithoutTax=il.amountWithoutTax+:deltaAmountWithoutTax, il.amountWithTax=il.amountWithTax+:deltaAmountWithTax, il.amountTax=il.amountTax+:deltaAmountTax, il.quantity=il.quantity+:deltaQuantity, il.auditable.updated=:now, il.unitPrice=0 WHERE il.id=:id"),
		...
		@NamedQuery(name = "InvoiceLine.deleteInvoiceLine", query = "DELETE InvoiceLine il where il.id = :ilId"),
	...
	@NamedNativeQuery(name = "InvoiceLine.checkThresholdB2B",
			query = "select billingAccountId, invoiceLineIds from ( " +
						"select b.id as billingAccountId, string_agg(distinct concat(il.id, ''), ',') as invoiceLineIds, " +
							...
							"(case when (b.threshold_per_entity =0 and b.invoicing_threshold  is not null) then b.invoicing_threshold else (case when (ca.threshold_per_entity=0 and ca.invoicing_threshold is not null) then ca.invoicing_threshold else (case when (c.threshold_per_entity=0 and c.invoicing_threshold is not null) then c.invoicing_threshold else :invoicingThreshold end) end) end) as IT, " +
